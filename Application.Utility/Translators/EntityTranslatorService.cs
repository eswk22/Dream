...
namespace Application.Utility.Translators
{
    public class EntityTranslatorService : IEntityTranslatorService
    {
        ...
        public IList<TTarget> Translate<TTarget>(object[] source)
        {
            IList<TTarget> items = new List<TTarget>();
            foreach (object item in source)
            {
                items.Add((TTarget)Translate(typeof(TTarget), item));         
            }
